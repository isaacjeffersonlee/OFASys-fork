...
class OFAExecutor(ABC):
    ...
    @abstractmethod
    def forward_decoder(
        self, ofa_model: "GeneralistModel", prev_output_tokens, **kwargs
    ):
        raise NotImplementedError
...
class GeneralistModel(BaseModel):
    ...
    def initialize(self, global_dict: Dictionary):
        ...
        for (
            pooling_module_name,
            pooling_module_config,
        ) in self.cfg.extra_models.pooling.items():
            self.extra_models[pooling_module_name] = OFAPoolingModel(
                pooling_module_config, global_dict, decoder.adaptor
            )
    ...
    def get_normalized_probs(
        ...
        return self.active_executor.get_normalized_probs(
            self, net_output=net_output, log_probs=log_probs, sample=sample
        )
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        if loaded_dict_size < len(self.encoder.dictionary):
            ...
            state_dict["encoder.adaptor.embed_tokens.weight"] = torch.cat(
                [
                    state_dict["encoder.adaptor.embed_tokens.weight"],
                    new_token_embed_to_add,
                ]
            )
            state_dict["decoder.adaptor.embed_tokens.weight"] = torch.cat(
                [
                    state_dict["decoder.adaptor.embed_tokens.weight"],
                    new_token_embed_to_add,
                ]
            )
    ...
    def update_embedding(self, state):
        if "global_dict_indices" not in state:
            return
        assert (
            "global_dict_indices" in state
        ), "Cannot find global_dict in restored ckpt!"
        ...
        idx = [
            self.global_dict.indices.get(token[0], len_dict) for token in tokens_sorted
        ]
        encoder_embedding = torch.zeros(len_dict + 1, emb_dim)
        torch.nn.init.normal_(encoder_embedding, mean=0, std=emb_dim**-0.5)
        encoder_embedding.to(
            dtype=state["model"]["encoder.adaptor.embed_tokens.weight"].dtype
        )
        encoder_embedding.index_copy_(
            0, torch.tensor(idx), state["model"]["encoder.adaptor.embed_tokens.weight"]
        )
        state["model"]["encoder.adaptor.embed_tokens.weight"] = encoder_embedding[
            :-1, :
        ]
        state["model"]["decoder.adaptor.embed_tokens.weight"] = encoder_embedding[
            :-1, :
        ]
