...
class BaseDataclass:
    ...
    def from_dict(cls, dict_data: dict):
        return from_dict(data_class=cls, data=dict_data, config=Config(cast=[Enum]))
...
@dataclass
class DistributedTrainingConfig(BaseDataclass):
    ...
    distributed_init_method: Optional[str] = field(
        default=None,
        metadata={"help": "typically tcp://hostname:port that will be used to " "establish initial connetion"},
    )
...
class DatasetConfig(BaseDataclass):
    ...
    max_valid_steps: Optional[int] = field(
        default=None, metadata={'help': 'How many batches to evaluate', "argparse_alias": "--nval"}
    )
...
@dataclass
class OptimizationConfig(BaseDataclass):
...
@dataclass
class FairseqBMUFConfig(BaseDataclass):
...
@dataclass
class GenerationConfig(BaseDataclass):
    beam: int = field(
        default=5,
...
@dataclass
class CommonEvalConfig(BaseDataclass):
    ...
    post_process: Optional[str] = field(
        ...
        metadata={
            "help": (
                ...
                "Valid options can be found in ofasys.preprocessor.data_utils.post_process."
            ),
...
@dataclass
class EvalLMConfig(BaseDataclass):
...
@dataclass
class InteractiveConfig(BaseDataclass):
...
class TrainerConfig(BaseDataclass):
    ...
    def from_yaml(cls, yaml_path):
        ...
        config_path, config_name = os.path.dirname(yaml_path), os.path.basename(yaml_path)

        GlobalHydra.instance().clear()
        with hydra.initialize_config_dir(version_base=None, config_dir=config_path):
            composed_cfg = hydra.compose(config_name)
...
cs.store('trainer', node=TrainerConfig)
